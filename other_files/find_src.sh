...
find "$(realpath "$PWD")" $@ \( -type f -name '*.[cChHsSi]' -o -name '*.c[cp]' -o -name '*.[ch]xx' -o -name '*.[ch]pp' -o -name '*.[CH]PP' -o -name '*.[ch]++' -o -name '*.tcc' -o -name '*.ii' -o -name '*.sx' -o -name '*.h[hp]' \) -print | tee /tmp/find_src_sh_results
